...
const { consumePauseRequest, pauseAndWaitForResume } = require("./pause");
...
// "Live Session" items (confirmed live) are Moodle mod_url resources that
// wrap a YouTube embed in a Video.js player, using its "youtube" tech. That
// wrapper doesn't create or populate a real <video> element at all until its
// own big-play-button is clicked — findVideo will see nothing (or an empty,
// unstarted <video> inside the YouTube iframe) until this fires. This is
// separate from the "Video N:" items, which are directly playable without
// any extra click. Cheap to check every iteration since it just returns
// false on pages that don't have this button.
async function clickPlayButtonIfPresent(page) {
  for (const frame of page.frames()) {
    let handle;
    try {
      ...
      try {
        ...
        await frame.evaluate((el) => el.click(), element).catch(() => {});
      }
      await element.dispose().catch(() => {});
      log("info", "VIDEO", "Clicked embedded player's play button.");
      return true;
    } catch (err) {
...
async function waitForVideoEnd(frame, handle) {
  ...
  while (Date.now() < hardDeadline) {
    if (consumePauseRequest()) {
      await frame.evaluate((v) => v.pause(), handle).catch(() => {});
      const choice = await pauseAndWaitForResume();
      if (choice === "close") return "closed";
      await frame.evaluate((v) => v.play().catch(() => {}), handle).catch(() => {});
      lastProgressAt = Date.now(); // paused time isn't a stall
      continue;
    }
...
module.exports = { findVideo, configureAndPlay, waitForVideoEnd, clickPlayButtonIfPresent };
