...
// first still-collapsed "Module N: ..." accordion header (Bootstrap-style:
// role="button", href="#collapseNNNNN", class contains "collapsed") in
// document order and clicks it, so later modules become reachable once
// earlier ones are done. Deliberately scoped to "Module N:" — the accordion
// also has non-module sections ("Course Overview", "Assessment",
// "Certificate") that match the same structural pattern but have no
// video/PDF/View content findClickable would ever recognize; expanding those
// was confirmed live to just waste a cycle finding nothing. Document order
// naturally means earlier modules get revisited first if they still have
// incomplete content, and only later ones get expanded once everything
// currently visible is exhausted.
    ...
    try {
      handle = await frame.evaluateHandle(() => {
        ...
        return headers.find((el) => {
          ...
          return /\bcollapsed\b/.test(cls) && /^Module\s+\d+\s*:/i.test(text);
        }) || null;
      });
