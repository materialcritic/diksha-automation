...
// which isn't consistent across courses.
//
// Each header is tagged with data-diksha-expand-tried the moment it's picked,
// so a header that doesn't actually lose its `collapsed` class can't be
// selected forever on the same page load — that was the shape of the
// infinite-loop bug (expanding resets idlePasses, a fresh page load collapses
// everything again, so the idle counter could never reach its threshold). The
// tag is lost on reload, which is correct — after a page.goto everything
// really does need expanding again.
  ...
  for (const frame of page.frames()) {
    ...
    try {
      handle = await frame.evaluateHandle(() => {
        ...
        return (
          headers.find((el) => {
            if (el.getAttribute("data-diksha-expand-tried") === "1") return false;
            const cls = typeof el.className === "string" ? el.className : "";
            const text = (el.textContent || "").trim();
            if (!/\bcollapsed\b/.test(cls)) return false;
            if (!text || skip.test(text)) return false;
            el.setAttribute("data-diksha-expand-tried", "1");
            return true;
          }) || null
        );
      });
      ...
      const label = await frame
        .evaluate((el) => (el.textContent || "").replace(/\s+/g, " ").trim(), element)
        .catch(() => "");
...
// Accordion bodies are populated by an async request that fires after the
// header is clicked. A fixed sleep was the wrong tool — confirmed live, a View
// button stayed invisible to findClickable through six full expansion cycles
// (~40s) and then appeared immediately after a human pause. Poll the count of
// content-bearing nodes instead and return once it's been stable across two
// consecutive checks, so a slow section is waited out and a fast one isn't.
async function waitForContentSettled(page, timeoutMs = cfg.SECTION_SETTLE_MS) {
        ...
        .evaluate(
          () =>
            document.querySelectorAll(
              ".module-action-btns a, .module-action-btns button, .modules_accordian_content [data-id], .module-progress-pie [title]"
            ).length
        )
        .catch(() => 0);
    }
...
// Expands every collapsed content section on the currently-loaded page, waiting
// for each one's content to settle before moving on. Called after every page
// load, so the main loop always scans a fully-expanded page rather than
// discovering sections one at a time across iterations — which was the shape
// that let the expand/reload cycle run forever.
async function expandAllSections(page, { maxSections = cfg.MAX_SECTIONS_PER_PAGE } = {}) {
  let expanded = 0;
  for (let i = 0; i < maxSections; i++) {
    if (!(await expandNextModule(page))) break;
    expanded++;
    await waitForContentSettled(page);
  }
  if (expanded) log("debug", "MODULE", `Expanded ${expanded} section(s) on this page load.`);
  return expanded;
}
